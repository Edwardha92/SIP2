...
%         akf_window = [akf_window tmp];
        ...
        [dat, idx] = process(tmp, filter_coeff);
        data = [data; dat];
        index = [index; idx]; 
    end
end 

function [downsampled_data, downsampled_idx] = process(akf_window, filter_coeff)
    max_values = [];
    max_idx = [];
    ...
    filtered_max = filter(filter_coeff, [1], max_values);
    
    downsampled_data = [max_values(1) downsample(filtered_max, 9, 4)];
